...
test.describe('Notes', () => {
  ...
  test('should add a new note', async ({ page }) => {
    ...
    await expect(page.locator('.note .note-content p:has-text("新しいメモ")')).toBeVisible();
  });

  test('should delete a note', async ({ page }) => {
    await page.fill('textarea[placeholder="新しいメモを入力"]', '削除するメモ');
    await page.click('button:has-text("追加")');
    await page.click('.note:has(.note-content p:has-text("削除するメモ")) .action-button.delete');
    await expect(page.locator('.note:has(.note-content p:has-text("削除するメモ"))')).not.toBeVisible();
  });
  ...
  test('should display multiple notes', async ({ page }) => {
    ...
    await expect(page.locator('.note .note-content p:has-text("メモ1")')).toBeVisible();
    await expect(page.locator('.note .note-content p:has-text("メモ2")')).toBeVisible();
    await expect(page.locator('.note .note-content p:has-text("メモ3")')).toBeVisible();
  });
