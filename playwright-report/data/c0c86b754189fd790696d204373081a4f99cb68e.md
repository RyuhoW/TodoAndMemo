...
- Name: Dashboard Component >> ダッシュボードの基本表示
- Location: C:\Users\ryuho\SynologyDrive\Projects\TodoAndMemo\e2e\dashboard.spec.ts:12:7

# Error details

```
Error: expect.toBeVisible: Error: strict mode violation: getByText('ダッシュボード') resolved to 2 elements:
    1) <button class="active">ダッシュボード</button> aka getByRole('button', { name: 'ダッシュボード' })
    2) <h2>ダッシュボード</h2> aka getByRole('heading', { name: 'ダッシュボード' })

Call log:
  - expect.toBeVisible with timeout 5000ms
  - waiting for getByText('ダッシュボード')

    at C:\Users\ryuho\SynologyDrive\Projects\TodoAndMemo\e2e\dashboard.spec.ts:15:45
```
...
- main:
  ...
  - list:
    ...
    - listitem:
      - img
      - text: 未完了
  - heading "週間の活動" [level=3]
  - img: 日 月 火 水 木 金 土 0 1 2 3 4
  - list:
    - listitem:
      - img
      - text: Todo
